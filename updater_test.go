...
func TestFindComposeProjects_MatchingContainer(t *testing.T) {
	...
	u := NewUpdater(nil, lister, nil, noopNotifier{}, false)
...
func TestHandleUpdate_NoProjects(t *testing.T) {
	...
	u := NewUpdater(up, lister, nil, noopNotifier{}, false)
...
func TestHandleUpdate_WithProject(t *testing.T) {
	...
	up := &mockComposeUpRunner{}
	u := NewUpdater(up, lister, loader, noopNotifier{}, false)
...
func TestFindComposeProjects_IncludeOnly_SkipsUnlabelled(t *testing.T) {
	lister := &mockContainerLister{
		containers: []container.Summary{
			{
				Image: "myapp:latest",
	...
	u := NewUpdater(nil, lister, nil, noopNotifier{}, true)
	projects, err := u.findComposeProjects(context.Background(), "myapp:latest", "")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if len(projects) != 0 {
		t.Fatalf("expected 0 projects without include label, got %d", len(projects))
	}
}

func TestFindComposeProjects_IncludeOnly_IncludesLabelled(t *testing.T) {
	lister := &mockContainerLister{
		containers: []container.Summary{
			{
				Image: "myapp:latest",
...
func TestFindComposeProjects_IncludeOnly_IgnoresExcludeLabel(t *testing.T) {
	lister := &mockContainerLister{
		containers: []container.Summary{
			{
				Image: "myapp:latest",
	...
	if len(projects) != 1 {
		t.Fatalf("expected 1 project (include overrides exclude), got %d", len(projects))
	}
}

func TestFindComposeProjects_TagMatch(t *testing.T) {
	lister := &mockContainerLister{
		containers: []container.Summary{
			{
				Image: "myapp:latest",
	...
	u := NewUpdater(nil, lister, nil, noopNotifier{}, false)
	projects, err := u.findComposeProjects(context.Background(), "myapp:latest", "latest")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestFindComposeProjects_TagMismatch(t *testing.T) {
	lister := &mockContainerLister{
		containers: []container.Summary{
			{
				Image: "myapp:latest",
	...
	u := NewUpdater(nil, lister, nil, noopNotifier{}, false)
	projects, err := u.findComposeProjects(context.Background(), "myapp:latest", "dev")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if len(projects) != 0 {
		t.Fatalf("expected 0 projects for tag mismatch, got %d", len(projects))
	}
}

func TestFindComposeProjects_UntaggedMatchesLatest(t *testing.T) {
	lister := &mockContainerLister{
		containers: []container.Summary{
			{
				Image: "myapp",
	...
	u := NewUpdater(nil, lister, nil, noopNotifier{}, false)
	projects, err := u.findComposeProjects(context.Background(), "myapp", "")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if len(projects) != 1 {
		t.Fatalf("expected 1 project (untagged=latest matches empty=latest), got %d", len(projects))
	}
}

func TestFindComposeProjects_TagFilterAcrossContainers(t *testing.T) {
	lister := &mockContainerLister{
		containers: []container.Summary{
			{
				Image: "myapp:dev",
				Labels: map[string]string{
					api.ProjectLabel:    "dev-project",
					api.WorkingDirLabel: "/opt/dev",
				},
			},
			{
				Image: "myapp:latest",
				Labels: map[string]string{
					api.ProjectLabel:    "prod-project",
					api.WorkingDirLabel: "/opt/prod",
				},
			},
		},
	}
	...
	u := NewUpdater(nil, lister, nil, noopNotifier{}, false)
	projects, err := u.findComposeProjects(context.Background(), "myapp:dev", "dev")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if projects[0].ProjectName != "dev-project" {
		t.Errorf("expected dev-project, got %s", projects[0].ProjectName)
	}
}
